...
async function create(result) {
  const sql = `
    INSERT INTO backtest_results (strategy_name, run_date, train_start, train_end, test_start, test_end, total_signals, wins, losses, neutral, win_rate_pct, avg_return_pct, max_drawdown_pct, sharpe_ratio, profit_factor, avg_holding_days, weight_config)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  const params = [
    ...
    result.win_rate_pct, result.avg_return_pct, result.max_drawdown_pct,
...
async function findAll({ page = 1, limit = 20, sort_by = 'run_date', sort_order = 'DESC', strategy_name } = {}) {
  ...
  const allowed_sort = ['run_date', 'win_rate_pct', 'sharpe_ratio', 'created_at'];
